...
overlap. All algebra (Spec 03) operates on this struct.
...
follows the DST rules in Spec 03.
...
%ExBooking.MeetingType{
  ...
  participants: :one | :collective | :pool, # who must be free (Spec 04)
...
%ExBooking.Request{
  ...
  routing_context: map(),               # OPAQUE. Never interpreted by the kernel;
                                        # surfaced to the scoring hook (Spec 04)
...
%ExBooking.Decision{
  ...
  events: [ExBooking.Event.t()],           # canonical events (Spec 05)
  intents: [intent()]                      # side effects for the consumer (Spec 05)
}
```
...
Canonical vocabulary consumed by orchestration, billing, and analytics (Spec 05).
