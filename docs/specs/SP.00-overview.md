...
ex_booking:
  ...
  created: "2026-07-08"
  updated: "2026-07-08"
  tags: ["overview", "scope", "determinism", "layering"]
  depends_on: ["R.01"]
---

# SP.00 — Overview

Normative · applies to `ex_booking` v0.x.
...
| CRM sync, enrichment, qualification forms, GTM analytics | consumer / recipe bundles |
| Payments, billing, invoicing, usage metering | adapters |
| Notifications (email/SMS/chat), UI, embeds | consumer |
| Multi-tenancy, auth, RBAC | consumer |
| AI features (prep, notetaking, phone agents) | consumer |
...
test strategy (SP.06). Any function that cannot satisfy them does not belong in
this library.

## Versioning

Semantic versioning against the public API defined in SP.02. Struct fields are
